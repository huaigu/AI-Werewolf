...
2. **存活状态检查**：你的target必须是存活玩家[${playerList}]中的一个
3. 如果你在reason中说"应该投票给X号"，那么target必须是X
4. 如果你在reason中说"保护Y号金水"，那么target绝不能是Y
5. 检查：你的JSON中的target数字是否正确对应你想投票的玩家？
6. 再次确认：你的target不在任何预言家的金水列表中？
7. 最后检查：投票给死人会导致投票无效，确认你没有犯这个错误？
...
## 【基础投票约束】
🔴 **必须满足的基础约束**：你的投票目标必须IN [${playerList}] - 只能投票给存活玩家！
🤝 **队友保护**：绝不投票给你的狼人队友[${teammates}]
⚠️ **伪装要求**：投票行为要符合好人身份，不要暴露狼人意图
...
1. 你的target数值必须与你reason中的最终结论完全一致
2. **存活状态检查**：你的target必须是存活玩家[${playerList}]中的一个
3. **队友保护检查**：你的target绝不能是你的狼人队友[${teammates}]
4. 检查：你的JSON中的target数字是否正确对应你想投票的玩家？
5. 最后检查：投票给死人或队友会暴露身份，确认你没有犯这些错误？
...
✅ **优先投票目标**：[${werewolfTargets.filter(id => context.alivePlayers.some(p => p.id === id)).join('、') || '暂无'}] - 这些是你验证且存活的狼人（查杀）

### 【致命错误防范】
- **投票给死亡玩家等同于系统错误，会导致投票无效**
- **投票给自己的金水等同于自爆狼人身份**
- **这会瞬间摧毁你在所有AI眼中的可信度**
- **即使金水发言有瑕疵，也绝不能投票他们**
- **金水是你的队友，必须无条件保护**
...
作为预言家，你的投票策略：
1. **【最高优先级】投票给你确认且存活的狼人[${werewolfTargets.filter(id => context.alivePlayers.some(p => p.id === id)).join('、') || '暂无'}]**
2. **【绝对禁令】绝不投票给你确认的好人[${goldWaterPlayers.join('、') || '暂无'}]**
3. **【绝对禁令】绝不投票给死亡玩家，只能从存活玩家[${playerList}]中选择**
4. **【次要选择】如无查杀目标，投票给"态势分析摘要"中高可疑度且存活的玩家**
5. **【团队领导】通过发言引导好人投票正确目标**

### 【投票目标合法性检查】
- 🔴 **存活状态检查**：你的投票目标必须IN [${playerList}]
- ⛔ **金水保护检查**：你的投票目标必须NOT IN [${goldWaterPlayers.join('、') || '无金水'}]
- ✅ **推荐投票目标**：存活查杀[${werewolfTargets.filter(id => context.alivePlayers.some(p => p.id === id)).join('、') || '无'}] > 存活高可疑度玩家 > 其他存活玩家
- ❌ **如果你想投票金水或死人，请立即重新考虑，这是致命错误**
...
1. 你的target数值必须与你reason中的最终结论完全一致
2. **存活状态检查**：你的target必须是存活玩家[${playerList}]中的一个
3. **金水保护检查**：你的target绝不能是你验证的任何好人[${goldWaterPlayers.join('、') || '无'}]
4. **查杀优先确认**：如果有存活查杀目标[${werewolfTargets.filter(id => context.alivePlayers.some(p => p.id === id)).join('、') || '无'}]，优先投票他们
5. 检查：你的JSON中的target数字是否正确对应你想投票的玩家？
6. 最后检查：投票给金水或死人等同于致命错误，确认你没有犯这些错误？
...
1. 你的target数值必须与你reason中的最终结论完全一致
2. **存活状态检查**：你的target必须是存活玩家[${playerList}]中的一个
3. **金水保护检查**：你的target绝不能是任何预言家的金水[${seerSituation.goldWaterPlayers.join('、') || '无'}]
4. **查杀优先确认**：如果有查杀目标[${seerSituation.werewolfTargets.join('、') || '无'}]，优先投票他们
5. **预言家保护检查**：在单预言家情况下绝不能投票预言家${seerSituation.selectedSeer || ''}号
6. 检查：你的JSON中的target数字是否正确对应你想投票的玩家？
7. **败局责任确认**：投票给金水/预言家/死人等同于背叛好人阵营，确认你没有犯这些致命错误？
